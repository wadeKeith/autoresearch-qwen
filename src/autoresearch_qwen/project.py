...
PROJECT_NAME = "autoresearch-qwen"
TAGLINE = "Training-code autoresearch for Qwen3-VL-4B on the official DocVQA splits (NVIDIA multi-GPU)."
...
THESIS_LINES = [
    ...
    "Optimize for repeated public-benchmark experiments with multi-GPU training.",
]

LOOP_LINES = [
    "autoresearch-qwen doctor checks Python, GPU, data, and local model readiness.",
    "prepare.py snapshot-downloads the full official DocumentVQA dataset.",
    "train.py learns on train with LoRA (default) or full SFT (multi-GPU via torchrun).",
    "evaluate.py runs the final visible validation score after training.",
    "evaluate.py exports blind test predictions when needed.",
    "run_experiment.sh is the one-command train plus eval loop.",
]
...
FILES_LINES = [
    ...
    "run_experiment.sh: isolated experiment runner (launches multi-GPU training).",
